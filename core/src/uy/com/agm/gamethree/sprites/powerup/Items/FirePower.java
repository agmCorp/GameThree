...
public class FirePower extends Item {
    private static final String TAG = FirePower.class.getName();

    private float stateTimer;
    private float stateWaiting;
    private float stateFading;
    private Animation firePowerAnimation;
    ...
    protected FirePower(PlayScreen screen, float x, float y, Animation firePowerAnimation, Animation fireAnimation, float fireDelay, int fireBullets) {
        super(screen, x, y);

        this.firePowerAnimation = firePowerAnimation;
    ...
    private void stateWaiting(float dt) {
        ...
        * At this time, FirePower may have collided with sth., and therefore, it has a new position after running the physical simulation.
        * In b2box the origin is at the center of the body, so we must recalculate the new lower left vertex of its bounds.
        * GetWidth and getHeight was established in the constructor of this class (see setBounds).
        * Once its position is established correctly, the Sprite can be drawn at the exact point it should be.
         */
        setPosition(b2body.getPosition().x - getWidth() / 2, b2body.getPosition().y - getHeight() / 2);
        setRegion((TextureRegion) firePowerAnimation.getKeyFrame(stateTimer, true));
        stateTimer += dt;
    ...
    private void stateTaken() {
        ...
        hud.setPowerLabel("FIRE MODE", 200); // todo
